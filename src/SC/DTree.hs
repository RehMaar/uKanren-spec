...
-- Derivation Tree
data DTree' a =
    Fail   -- | Failed derivation.
  | Success E.Sigma -- | Success derivation.
  | Or [DTree' a] E.Sigma a (Set.Set DGoal)            -- | Intermediate node, that contains a conjunctions.
                                                         -- | It's children are parts of a disjunction.
  | And [DTree' a] E.Sigma a  (Set.Set DGoal)           -- | Node for an abstracted goal.
  | UAbs [DTree' a] E.Sigma a  (Set.Set DGoal) [(S, Ts)] -- | Node for an upward abstracted goal. Also have to save arguments mapping.
...
countLeafs (Or ts _ _ _) = foldl
  (\(n1, m1, h1) (n2, m2, h2) -> (n1 + n2, m1 + m2, h1 + h2))
  (0, 0, 0)
  (countLeafs <$> ts)
countLeafs (And ts _ _ _) = foldl
  (\(n1, m1, h1) (n2, m2, h2) -> (n1 + n2, m1 + m2, h1 + h2))
  (0, 0, 0)
  (countLeafs <$> ts)
