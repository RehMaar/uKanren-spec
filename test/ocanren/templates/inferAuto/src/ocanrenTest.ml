...
let weakPierce () = arr (arr (arr (arr (arr (p varX) (p varY)) (p varX)) (p varX)) (p varY)) (p varY)

let () =
(*  runEnv   1   q   qh (REPR (fun q -> lookupo varX q (v varY)                                       ));*)
(*  runT     2   q   qh (REPR (fun q -> infero (v varX) q                                             ));
  runT     2   q   qh (REPR (fun q -> infero (abs varX (v varX)) q                                  ));
  runT     2   q   qh (REPR (fun q -> infero (abs varF (abs varX (app (v varF) (v varX)))) q        ));
  runT     2   q   qh (REPR (fun q -> infero (abs varX (abs varF (app (v varF) (v varX)))) q        ));*)
(*  runL     2   q   qh (REPR (fun q -> infero q (arr (p varX) (p varX))));*)
  runL     1   q   qh (REPR (fun q -> infero q (weakPierce ())))
